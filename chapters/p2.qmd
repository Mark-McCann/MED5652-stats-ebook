...
2. Locate a file you have downloaded, and state where your browser saves files by default
3. Read, construct, and copy a full file path on your own operating system
4. Import a CSV into R using a correctly specified path
5. Diagnose and resolve the most common import failures
6. Inspect an imported dataset's dimensions, variable names, and types
7. Identify missing values in a dataset
8. Produce a simple summary and a simple plot
9. Write, save, and re-run an R script that imports data, summarises it, and produces a plot

## Setup

There's nothing to read in yet, so, as in P1, this chunk only sets the console's print width.
...
**Screenshot placeholder** (`images/p2-messy-spreadsheet.png`): `p2_messy_clinic.xlsx` open in a spreadsheet application, showing the merged title row, the merged "Vitals" sub-header, the colour-only Smoker column, and the second table below the gap.
:::
...
This chapter needs three packages: `readr` for reading files, `here` for building paths, and `ggplot2` for the plot at the end. As P1 explained, installing a package and loading it are two different actions, so unless you've installed these already, run this once in the console, not inside this document:
...
The real file is called `mph_nhanes_20241107.csv`, no `_data` in the name. R's message states plainly that the file doesn't exist at the path it tried, which is exactly what P1's "file does not exist" error type looks like with a real file behind it.
...
The dictionary's first column is called `Variable names`, with a space in it, so referring to it needs backticks, exactly as P1 covered. `%in%`, from P1 as well, tests whether each value is a member of a set. Put inside square brackets after a data frame's column, with the row's index implied, this keeps only the rows where the condition is true:
...
`chol` is total cholesterol, in mmol/L, and `r format(sum(is.na(nhanes$chol)), big.mark = ",")` participants have no value recorded for it, the same kind of gap `vpa_w` had above. `na.rm = TRUE` drops those missing values before averaging the rest, exactly as it did back in P1's `mean()` example. Leave it out and the whole calculation returns `NA`, since R won't silently guess what a missing value should have been.

### A first taste of a grouped summary

`nhanes$chol` pulls the whole cholesterol column out as a bare vector, exactly as `$` did in P1. Adding square brackets straight after it, with a condition inside, keeps only the values where that condition is `TRUE`:
...
Everything we've done so far in this chapter has been one line at a time. That's the right way to *try* something: type it in the console, look at what comes back, fix it if it's wrong. But P1 already made the argument against leaving it there. You will need to do this again, and you will not remember what you typed.
...
3. Save it into the `scripts/` folder you built in P1, with a name that follows P1's conventions: no spaces, no `&` or `#`, something that says what it does. `first_analysis.R` is fine. `Untitled1.R` is not.
...
If it runs clean, the script is genuinely self-contained. If it fails, the error tells you precisely what you'd forgotten to write down, and the fix belongs *in the script*, not typed into the console again. That restart is the reproducibility argument from P1, made concrete: not "scripts are good practice" but "here is how you find out whether yours actually is one."
...
**Exercise 1** (ILOs 3, 4, 5). This `read_csv()` call is supposed to read the dictionary file but doesn't work:
...
**Exercise 2** (ILOs 6, 7). Check `mpa_w` (moderate physical activity at work): find its type with `class()`, and count how many values are missing with `is.na()` and `sum()`.

**Exercise 3** (ILO 8). Produce one summary statistic and one plot for `wal_r` (walking, hours/day, outside work). Remember `na.rm = TRUE` if it turns out to have missing values.

**Exercise 4** (ILO 9). Collect your answer to Exercise 3 into a proper script. Open a new R script, and write into it everything needed to go from a completely empty session to that summary and that plot: the `library()` calls, the `read_csv()` line, then the summary and the plot themselves. Save it into your `scripts/` folder with a sensible name.
