# Describing and visualising data (IES Week 7) {#sec-w1-practical}

This practical follows the IES Week 7 seminar of the same name. That seminar covered variable types, tables and plots for describing a single variable, and how to choose between mean and SD or median and IQR. In P1 and P2 we already covered installing R, finding our way around a project folder, and importing a spreadsheet into R. So in this session we go straight to using that data.

## Chapter intended learning outcomes

By the end of this session you will be able to:

1. Use `select()`, `filter()`, `mutate()`, and `arrange()` to manipulate a dataset
2. Produce grouped summaries with `group_by()` and `summarise()`
3. Construct frequency and cross-tabulations
4. Build histograms, boxplots, bar charts, and scatterplots in `ggplot2`
5. Modify a plot's bin width, scale, and labels, and describe the effect on interpretation
6. Convert a character variable to a factor and set its reference level
7. Produce a basic "Table 1" of participant characteristics
...
Running that in your own console prints more than it does on this page. The `library()` calls announce themselves, and each `read_csv()` prints the column specification message P2 unpacked. This book hides both from here on, because they say the same thing in every chapter and P2 already read one properly. Nothing is being kept from you: glance at the row and column counts in your own console the way P2 showed, and carry on.
...
`chol > 6` is a comparison, exactly like the ones in P1, and it returns `TRUE` or `FALSE` for every row. Participants with no cholesterol value recorded get `NA` rather than `FALSE`, which is correct: we don't know whether they're above 6, and guessing "no" would be inventing data.
