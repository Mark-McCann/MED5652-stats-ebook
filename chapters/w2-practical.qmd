# Estimation, precision, and study size (IES Week 8) {#sec-w2-practical}
...
1. Calculate the point estimate and its associated standard error of a mean and of a proportion from summary statistics
2. Construct a confidence interval by formula, and reproduce it using `t.test()` and `prop.test()`
3. Obtain confidence intervals for a difference in means and a difference in proportions
4. Interpret each interval in the units and context of the problem, in writing
5. Describe how an interval's width responds to sample size
6. Obtain a sample size for a two-group comparison using `power.prop.test()` and `power.t.test()`, and describe how the requirement changes with the assumed effect size
...
**Exercise 1** (ILOs 1, 2, 3, 4). Calculate the mean and standard error of total cholesterol (`chol`), construct a 95% confidence interval by hand, and confirm it with `t.test()`. Write one sentence reporting the result as it would appear in a results section.
...
**Exercise 2** (ILOs 1, 2, 3, 4). In `strep_tb`, find the proportion of patients with baseline cavitation present (`baseline_cavitation`) and calculate its standard error by hand, then build a 95% confidence interval from it and check the result against `prop.test()`. Then construct a 95% confidence interval for the difference in the proportion improved between men and women (`gender`). Write one interpretive sentence for each interval.

**Exercise 3** (ILOs 4, 5). Repeat the width exploration above (full sample, 50% subset, 10% subset) using `sed` instead of `chol`, and collect the three intervals into one table the way the chapter did, so their widths sit side by side. Describe what changes and what stays about the same as the sample size drops.

**Exercise 4** (ILOs 4, 6). Using `power.t.test()`, find the sample size needed to detect a difference in `chol` of 0.3 mmol/L, and again for a difference of 0.8 mmol/L, both at `power = 0.8`. Write one sentence explaining why the two required sample sizes differ so much.
