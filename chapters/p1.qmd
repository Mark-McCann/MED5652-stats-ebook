# R and R Studio (IES Week 5) {#sec-p1-practical}
...
1. Install R and RStudio on your own device
2. Identify the console, script editor, environment, and files panes, and describe what each is for
3. Create an RStudio Project and explain how it relates to the folder structure built in this chapter
4. Recognise R's basic data types and structures (numeric, character, logical, factor; vector, data frame), and build a vector with `c()`
5. Call a function using positional and named arguments, and explain what a default argument value is
6. Build a logical condition with comparison operators (`==`, `!=`, `<`, `>`, `<=`, `>=`), negate one with `!`, combine conditions with `&`, and test membership with `%in%`
7. Write and read a short piped sequence of steps with `|>`
8. Extract a single column from a data frame with `$`, and explain how this differs from keeping a column with `select()`
9. Install and load a package, and explain why these are two different actions
10. Read an error message and identify which of the common failure types it represents
11. Access a function's help page
...
**Screenshot placeholder** (`images/p1-panes.png`): RStudio's default layout with all four panes labelled.
:::
...
::: {.callout-tip appearance="simple"}
**Screenshot placeholder** (`images/p1-new-project.png`): the New Project wizard, with Existing Directory highlighted.
:::

A Project's working directory is the folder R treats as "here" when it looks for a file. Once we start reading data in P2, this is what lets a path like `data/mph_nhanes_20241107.csv` work on any machine, no matter where the project folder actually sits on that machine's disk.
...
A vector holds one type of value, several at once. A **data frame** holds several vectors side by side, as columns, which is the structure this course actually works with from P2 onward. Building one from two vectors previews what's coming:
...
`data.frame()` normally cleans up column names automatically, turning a space into a dot. `check.names = FALSE` switches that off, purely so this example can show a genuine spaced name. When we read a CSV file with `read_csv()` in P2, column names come in exactly as written, spaces and all, with no automatic cleanup. P2's dictionary file has a column literally called `Variable names`, and backticks are exactly how we'll refer to it.
...
The warning line above states the actual reason, a missing file, before the error itself. We'll meet this one properly in P2, once there's a real file to read. `read.csv()` is base R's own version of the function; from P2 onward we use `read_csv()` from the `readr` package instead, which behaves a little differently but fails the same way when a path is wrong.

## Getting help

Every function used in this course has a help page. Typing `?mean` in the console and pressing Enter opens `mean()`'s help page in the Help pane, bottom-right, showing exactly what arguments it takes and what it returns. It's usually faster than searching online, and it's always correct for the version of R actually installed. RStudio also keeps a set of cheat sheets under **Help → Cheatsheets**, one per major package. Beyond that, this book itself stays the main reference for the rest of the course. P2 explains how to find your way around it.
...
**Exercise 1** (ILO 4). Create a vector called `heights` holding four numbers of your choosing, using `c()` and `<-`. Check its class with `class()`, and confirm `heights` appears in the Environment pane after you run your code.

**Exercise 2** (ILOs 5, 6, 11). Using `visits$weight_kg` from the worked example, round the values to one decimal place, naming the `digits` argument explicitly rather than relying on position. Then build a logical condition that identifies which visits are for a smoker weighing over 90 kg, combining a comparison operator with `&`.
...
**Exercise 3** (ILOs 7, 8). Write a piped sequence, using `|>`, that takes `visits$weight_kg`, rounds it to zero decimal places, and sums the result. Then use `select()` to keep just `visit_id` and `smoker` from `visits`, and write one sentence stating how the result differs from `visits$smoker`.
