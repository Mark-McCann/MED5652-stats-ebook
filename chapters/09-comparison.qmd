...
1. Perform and interpret an independent-samples t-test in R using `t.test()`
...
3. Construct a contingency table and perform a chi-squared test using `table()` and `chisq.test()`
4. Obtain risk ratios and odds ratios with confidence intervals using `epitools::riskratio()` and `epitools::oddsratio()`
5. Write up a result in an appropriate academic style
