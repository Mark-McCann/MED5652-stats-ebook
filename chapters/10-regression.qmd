...
Chapter 5 compared two groups directly against each other. But some public health questions aren't about groups at all: does cholesterol change with age, or with hours of sedentary time? How do we establish a relationship between two continuous variables in R, and know whether a straight line is a fair description of the relationship at all? And once we have a line, how do we use it to predict a value we haven't observed, while recognising when a prediction has wandered somewhere the data can't support?

We describe the relationship before modelling it, with a scatterplot and Pearson's correlation coefficient, plus Spearman's rank correlation for the case where one variable is ordered categories rather than a measurement. Then we model it, fitting a simple linear regression, Y = α + βX + ε: an outcome Y as an intercept α plus a slope β times a predictor X, with ε standing for the error the line doesn't capture. That turns the picture into an intercept, a slope with a confidence interval, and an R² saying how much of the outcome's variation was accounted for. Along the way, fitting that same model with a two-category predictor instead of a continuous one shows that Chapter 5's independent-samples t-test was this model all along, seen from a different angle. We check the regression assumptions with residual and QQ plots, so we know whether the model is robust. Then we use the model to predict, and see what it returns when we ask it about an input the data never contained.
